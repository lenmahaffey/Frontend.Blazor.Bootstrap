...
namespace Blazor.Frontend.Components.Charts
{
    ...
    {
        [Parameter] public List<BarChartData> displayData { get; set; } = new List<BarChartData>();
        [Parameter] public string xAxisName { get; set; } = "x-axis";
        [Parameter] public string yAxisName { get; set; } = "y-axis";
        ...
        private async Task RenderManhattanAsync()
        {
            ...
            {
                Labels = this.displayData.Select(x => x.Name).ToList(),
                Datasets = new List<IChartDataset>()
                {
                    new BarChartDataset()
                    {
                        Data = displayData.Select(x => x.Value).ToList(),
                        BackgroundColor = new List<string>{ "rgb(88, 80, 141)" },
                        CategoryPercentage = 0.8,
                        BarPercentage = 1,
                        //HoverBackgroundColor = new List<string>{ "rgb(8, 8, 14)" },
                        //BorderColor = new List<string>{ "rgb(8, 8, 14)" },
                        //BorderWidth = new List<double>{ 5.0 }
                    }
                }
            };
            ...
            options.Plugins.Title.Font.Size = 40;
